...
| F17 — Fonti collegate alle singole affermazioni | da fare | 25 agosto 2026 | Problema riverificato con numeri reali (99/157 schede con 3+ fonti indistinte) e brief scritto in `BRIEF-F17.md`, con due strade e criterio di accettazione — in attesa di scelta dell'autore |
...
| F32 — Modulo "proponi una canzone" | da fare | 25 agosto 2026 | Oggi esiste solo "Segnala un errore" (mailto). Un secondo link mailto per proposte di nuove canzoni è a basso sforzo, stesso stile di quello esistente |
...
| F34 — Player Spotify al posto dello spazio immagine | da fare | 26 agosto 2026 | Il riquadro "Spazio immagine" occupa la posizione migliore della scheda senza dare nulla al lettore. Attenzione: solo 75 canzoni su 157 hanno `spotifyId`, le altre 82 devono continuare a funzionare col riquadro attuale |
| F35 — "La storia" sotto il titolo | bloccato, attende l'autore | 26 agosto 2026 | Difetto reale verificato: `ganci.json` non esiste (0/157), quindi la sintesi sotto il titolo è un troncamento di `corpo[0]` e **la stessa frase compare due volte**. Ma spostare la storia sopra il momento iconico cambia l'ordine approvato in Costituzione (P4): due letture possibili, la scelta spetta all'autore |
| F36 — La pillola del giorno non cambia mai | da fare | 26 agosto 2026 | Bug verificato: `Date.now()` in `paginaHome` viene eseguito a build time, non nel browser — la pillola cambia solo quando si ripubblica. Va spostata la rotazione lato client (103 candidate con `fraseIconica`) e aggiunto il pulsante "Un'altra canzone" |
...
| F38 — Marchio nelle immagini condivisibili | da fare | 26 agosto 2026 | Nelle 157 `og:image` il logo è a 228px su 1200×630 e i righi musicali si perdono; il nome del sito è in un angolo, scollegato dal logo. Da rivedere scala e posizione, senza toccare il disegno |
